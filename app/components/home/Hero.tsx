...
export function Hero() {
  const { hero } = homeData
  return (
    <section className="hero relative min-h-160 overflow-hidden bg-[#fdf6f0] max-md:min-h-172.5">
      <div className="site-container hero-inner relative mx-auto min-h-160 w-full max-w-315 px-4 md:px-6 max-md:min-h-[690px]">
        <div className="hero-copy w-[54%] pt-16 max-lg:w-[61%] max-md:w-full max-md:pt-11">
          <span className="hero-kicker inline-block rounded-full bg-amber-50 px-3 py-2 text-[11px] font-bold text-amber-600">
            {hero.kicker}
          </span>
          ...
          <p className="max-w-132.5 text-sm leading-7 text-stone-600">
            {hero.description}
          </p>
          ...
          <div className="social-proof mt-6 flex flex-wrap items-center gap-3">
            <div className="avatars flex pl-2">
              {hero.socialProof.avatars.map((avatar) => (
                <span key={avatar}>{avatar}</span>
              ))}
            </div>
            <div className="flex flex-col">
              <b className="text-xs">{hero.socialProof.title}</b>
              <small className="text-[10px] text-stone-500">
                {hero.socialProof.subtitle}
              </small>
            </div>
            <div className="rating ml-2 flex gap-0.5 text-amber-400">
              {Array.from({ length: hero.socialProof.rating }, (_, index) => (
                <Star
                  className="h-3 w-3"
                  fill="currentColor"
                  key={index}
